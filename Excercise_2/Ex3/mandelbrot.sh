...
out_csv="./results/mandelbrot_execution_times.csv"
...
for pixels in "${pixel_values[@]}"; do
    ...
    for iterations in "${iteration_values[@]}"; do
        ...
        execution_time=$( { time -p ./build/main $pixels $pixels -2.25 -1.5 0.75 1.5 $iterations 2>&1; } 2>&1 | grep real | awk '{print $2}' )
