...
out_csv="/u/dssc/mzampar/High_Performance_Computing_2023/Excercise_1/barrier/results/barrier_recursive_doubling.csv"
...
for map in $map_values; do
  for np in $np_values; do
    ...
    mpirun -np $np -map-by $map --mca coll_tuned_use_dynamic_rules true --mca coll_tuned_bcast_algorithm 3 ${src_path}osu_barrier -x 1000 -i 10000 -f | tail -n 21 \
    | awk -v np="$np" -v map="$map" '{printf "RecursiveDoubling,%s,%s,%s,%s,%s,%s\n",map,np,$1,$2,$3,$4}' | sed 's/,$//' >> $out_csv
  done
done
