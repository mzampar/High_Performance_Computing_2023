...
out_csv="/u/dssc/mzampar/hpc_project/Excercise_1/scatter/results/scatter_basic_linear.csv"
...
for map in $map_values; do
  for np in $np_values; do
    ...
    echo "   Benchmarking Basic_Linear with map=$map and np=$np"
    mpirun -np $np -map-by $map --mca coll_tuned_use_dynamic_rules true --mca coll_tuned_bcast_algorithm 1 ${src_path}osu_scatter -x 1000 -i 10000 | tail -n 21 \
    | awk -v np="$np" -v map="$map" '{printf "Basic_Linear,%s,%s,%s,%s\n",map,np,$1, $2}' | sed 's/,$//' >> $out_csv
  done
done
