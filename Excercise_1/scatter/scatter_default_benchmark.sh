...
out_csv="/u/dssc/mzampar/hpc_project/Excercise_1/scatter/results/scatter_default.csv"
...
for map in $map_values; do
  for np in $np_values; do
    ...
    mpirun -np $np -map-by $map --mca coll_tuned_use_dynamic_rules true --mca coll_tuned_bcast_algorithm 0 ${src_path}osu_scatter -x 1000 -i 10000 | tail -n 21 \
